...
function clusterSessions(sessions, states, nrClusters){
  ...
  for(var i = 0; i < sessions.length; i++){
    if(sessions[i].length >= minLength && sessions[i].length <= maxLength)
      clusters[assignment[i]].push(sessions[i]);//.slice(0, sessions[i].length - 1));
  }
